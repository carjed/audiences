...
nsoc <- sddf2 %>% dplyr::filter(grepl("10.1101", doi)) %>% dplyr::filter(grepl("socialist", top_10)) %>% summarise(tot=sum(n))
nnat <- sddf2 %>% dplyr::filter(grepl("10.1101", doi)) %>% dplyr::filter(grepl("nationalist", top_10) & grepl("white", top_10)) %>% summarise(tot=sum(n))
...
**Audiences** analyzes the various communities and networks that are discussing [bioRxiv](https://biorxiv.org) preprints on Twitter. Our goal is to help authors better understand who is engaging with their work.

Click on the tabs above to view various summaries of the preprints analyzed.

-------

- Number of preprints indexed: **`r nrow(summary_data_df)`**
...
These analyses are described in detail in the following preprint:

> Carlson J, Harris K. The mismeasure of tweets: quantifying and contextualizing the societal impact of bioRxiv preprints through social media audience segmentation. *Journal.* 2020. [`doi:10.1186/s12864-018-5264-y`](http://dx.doi.org/10.1186/s12864-018-5264-y)
...
Of the **`r nrow(summary_data_df)`** preprints analyzed, our method estimates a higher fraction of the audiences are scientists than the Altmetric demographics for **`r n_acad_higher`** **(`r paste0(round((n_acad_higher/nrow(summary_data_df))*100), "%")`)** of these. 

According to the Altmetric demographics, **`r n_am_50`** of these preprints **(`r paste0(round((n_am_50/nrow(summary_data_df))*100), "%")`)** are tweeted primarily by non-scientist audiences; our method estimates only **`r n_aud_50`** papers **(`r paste0(round((n_aud_50/nrow(summary_data_df))*100), "%")`)** are primarily tweeted by non-scientist audiences.
...
```{r beeswarm, echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.height=8, fig.width=12}
...
nc1 <- sddf2 %>%
  ...
  mutate(pval=prop.test(c(cons, lib), c(tot_tweets, tot_tweets))$p.value) %>%
  full_join(data.frame("categories_trim"=categories_trim), by="categories_trim") %>%
  mutate(prop=(cons+lib)/tot_tweets)

p <- ggplot(nc1, aes(x=log10(cons/lib), y=-log10(pval), label=categories_trim, fill=categories_trim))+
  ...
  xlab(expression("log"[10]*"(fold difference)"))+
