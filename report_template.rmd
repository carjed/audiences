...
params: 
  ...
  abstract: ""
  doi: ""
  article_full_url: "https://biorxiv.altmetric.com/details/52355933"
...
datadir <- rprojroot::find_root(".here")
...
One of the challenges in altmetrics audience analysis is detailed classification of the various groups of users who are interested in a paper on social media. This is due to the limited information available about each user—Twitter limits user biographies to 160 characters.
...
### Scrape follower lists and bios

The code below will scrape data from Altmetric and Twitter to get information about each user that has tweeted or RTed the specified article, and the handles of each user's followers.
...
```{r fetch_am_data, warning=FALSE, message=FALSE, error=FALSE}
#-----------------------------------------------------------------------------
...
Next, we scrape the bios of the followers (limited to the 10,000 most recent followers) for each account that has tweeted/RTed a reference to the article and compile these bios into a single "document" per account. We calculate the frequencies of every term that occurs 10 or more times in each document, excluding common stop words. Note that because emoji and hashtags are commonly used to convey semantic meaning in a user's bio, we include these as unique "words".

From the word frequency data, we then generate a document term matrix, which serves as the input for the LDA model.

```{r get_follower_bios, warning=FALSE, message=FALSE, error=FALSE}
...
ggsave(paste0(datadir, "/static/figures/", gsub(".html", ".png", nb_file)), 
