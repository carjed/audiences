...
user_data <- lookup_users(handles) %>%
  dplyr::filter(!(account_lang %in% c("ar", "ja", "zh-CN", "ko")) & protected==FALSE)
...
if(file.exists(altmetric_data_full_fh)){
  ...
  for(user in (nrow(altmetric_data_full)+1):nrow(user_data)){
    ...
    # } else if (user_data[user,]$lang != "en"){
    #   cat(paste0("User profile for ", user_data[user,]$screen_name, " is not in English—skipping\n"))
    # } else {
